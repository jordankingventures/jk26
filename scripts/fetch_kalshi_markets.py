...
Haalt de actuele Kalshi-weekly-views-markten op (series KXYTVIEWSW,
"Highest daily view count this week", 1 event per artiest) met hun live
YES/NO-prijzen per bracket, en schrijft dat naar data/kalshi_markets.json.
...
def artist_key_for_event_ticker(event_ticker):
...
def fetch_events():
    data = get_json("/events", {"series_ticker": SERIES_TICKER, "status": "open", "limit": 100})
